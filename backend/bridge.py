...
# ── ВАЖНО: load_dotenv НЕ должен перебивать PORT ──────────────
# Firebase Studio передаёт PORT через os.environ ДО запуска.
# override=False означает: если переменная уже есть в окружении
# (от Studio) — не трогаем её. .env может содержать PORT=5000,
# но Studio уже выставила PORT=5005 — оставляем Studio-значение.
...
# Добавлено для проверки здоровья (health check) корня от Studio
@app.route('/', methods=['GET'])
def root_health():
    """Корневой health-check для Firebase Studio."""
    return jsonify({'status': 'ok', 'message': 'Bridge is running'}), 200
...
if __name__ == '__main__':
    ...
    # Убиваем зависший процесс на этом порту.
    # fuser доступен т.к. pkgs.psmisc прописан в idx.nix.
    try:
        kill = subprocess.run(['fuser', '-k', f'{port}/tcp'],
    ...
    # use_reloader=False — КРИТИЧНО для Firebase Studio previews:
    # Flask reloader форкает дочерний процесс => два процесса на порту.
    # Studio видит двойной старт и пытается перезапустить снова.
    app.run(
        host='0.0.0.0',
        port=port,
        debug=False,
