...
/* ══════════════════════════════════════════════════
   ЛИЧНЫЙ МЕССЕНДЖЕР — открыть / закрыть
══════════════════════════════════════════════════ */
function openMessenger() {
    var modal = document.getElementById('messenger-modal');
...
/* ══════════════════════════════════════════════════
   FARADAY AI ЧАТ — открыть / закрыть
══════════════════════════════════════════════════ */
...
function sendFaradayMessage() {
    ...
    setTimeout(function() {
        ...
        appendFaradayAIMsg(feed, response);
    }, 500 + Math.random() * 400);
}
...
function processFaradayCommand(text) {
    ...
    if (t.includes('смени цвет') || t.includes('change color') || t.includes('цвет на')) {
        var newColor = '#00ff88';
        for (var key in colorMap) {
            if (t.includes(key)) { newColor = colorMap[key]; break; }
        }
    ...
    if (t.includes('пауза') || t.includes('стоп') || t.includes('pause') || t.includes('stop')) {
        ...
        if (versionPill) versionPill.innerText = 'ПАУЗА';
        db.collection('system_config').doc('faraday_protocol')
    ...
    // ── Запуск / активировать ──
    if (t.includes('активируй') || t.includes('пуск') || t.includes('запуск') || t.includes('activate') || t.includes('start')) {
        window.faradaySystemPaused = false;
        var vid2 = document.getElementById('bg-video');
        if (vid2) vid2.play().catch(function() {});
        setStatus('SYSTEM: ACTIVE');
        if (versionPill) versionPill.innerText = 'АКТИВЕН';
        db.collection('system_config').doc('faraday_protocol')
    ...
    if (t.includes('синхронизация') || t.includes('sync') || t.includes('обнови')) {
        setStatus('SYNCING...');
        db.collection('system_config').doc('faraday_protocol').get()
          .then(function(snap) {
              if (!snap.exists) { setStatus('SYSTEM: STANDBY'); return; }
              var cfg = snap.data();
    ...
    if (t.startsWith('запомни') || t.startsWith('remember')) {
        var memData = text.replace(/запомни|remember/i, '').trim();
        if (memData) {
        ...
        return 'Что именно запомнить? Напишите: «Запомни [текст]»';
    }
...
/* ══════════════════════════════════════════════════
   ЛИЧНЫЙ ЧАТ — рендер сообщений Firebase
══════════════════════════════════════════════════ */
...
function handleLogout() {
    auth.signOut().catch(function(err) { console.error('Logout error:', err); });
}
...
function initFaradayCore() {
    ...
    faradayProtocolRef.onSnapshot(function(snapshot) {
        ...
        if (versionEl && config.version) versionEl.innerText = 'Ver: ' + config.version;
...
function getStatusText(status) {
    if (status === 'online')  return T[currentLang].status_online  || 'ONLINE';
    if (status === 'busy')    return T[currentLang].status_busy    || 'BUSY';
    return T[currentLang].status_offline || 'OFFLINE';
}
...
function showTooltip(memberIndex, cardEl) {
    ...
    var avatarEl = document.getElementById('ctt-avatar');
    var nameEl   = document.getElementById('ctt-name');
    var roleEl   = document.getElementById('ctt-role');
    var statusEl = document.getElementById('ctt-status');
    var tagsEl   = document.getElementById('ctt-tags');

    if (avatarEl) avatarEl.innerText = member.emoji;
    if (nameEl)   nameEl.innerText   = member.name;
    if (roleEl)   roleEl.innerText   = currentLang === 'ru' ? member.role_ru : member.role_en;
    if (statusEl) statusEl.innerText = getStatusText(member.status);
    if (tagsEl)   tagsEl.innerHTML   = member.tags.map(function(t) { return '<span>' + t + '</span>'; }).join('');
...
function initCarousel() {
    ...
    var pauseBtn = document.getElementById('car-pause');
    ...
    if (prevBtn) prevBtn.addEventListener('click', function() { angle += (2 * Math.PI / N); });
    if (nextBtn) nextBtn.addEventListener('click', function() { angle -= (2 * Math.PI / N); });
}

/* ══════════════════════════════════════════════════
   NITRO BOOST — счётчики
══════════════════════════════════════════════════ */
